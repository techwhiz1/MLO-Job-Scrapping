#!/usr/bin/env python3
"""
SSL Setup for mininglifeserver.com
Configure domain with SSL certificate and Nginx proxy for port 8888
"""
...
def setup_firewall():
    ...
    commands = [
        ...
        "sudo ufw allow 8888",  # Allow direct port 8888 access
        "sudo ufw allow ssh",
...
def create_nginx_config(domain, backend_port=8888):
    """Create Nginx configuration for SSL proxy on port 8888"""
    config_content = f"""
...
# Standard HTTPS server on port 443 (optional)
server {{
...
def update_backend_cors(domain):
    """Update backend CORS configuration for new domain"""
    print(f"🔧 Updating backend CORS for {domain}...")
    
    # This would require updating the backend/main.py file
    # For now, we'll just print the instructions
    print(f"📝 Manual step: Update backend/main.py CORS origins to include:")
    print(f"   - https://{domain}")
    print(f"   - https://{domain}:8888")
    print(f"   - https://mininglifeonline.com")
    
    return True

def test_https_endpoint(domain):
    """Test the HTTPS endpoint"""
    print(f"🧪 Testing HTTPS endpoint: https://{domain}:8888")
    
    test_commands = [
        f"curl -I https://{domain}:8888/",
        f"curl -I https://{domain}:8888/cors-test",
        f"curl https://{domain}:8888/",
        f"curl https://{domain}:8888/cors-test"
    ]
...
def main():
    """Main setup process"""
    print("🔐 SSL Setup for mininglifeserver.com")
    print("=" * 50)
    
    domain = "mininglifeserver.com"
    backend_port = 8888
    
    print(f"🌐 Domain: {domain}")
    print(f"🔧 Backend Port: {backend_port}")
    print(f"🎯 Target: https://{domain}:8888/scrape")
    ...
    print("5. Update backend CORS")
    print("6. Test HTTPS endpoint")
    
    print(f"\n⚠️  IMPORTANT: Make sure your domain {domain} points to this VPS IP!")
    print(f"   DNS A record: {domain} -> YOUR_VPS_IP")
    ...
    update_backend_cors(domain)
    
    # Step 6: Test HTTPS endpoint
    test_https_endpoint(domain)
    
    print(f"\n🎉 SSL Setup Complete!")
    print(f"✅ Backend HTTPS URL: https://{domain}:8888")
    print(f"✅ API Endpoint: https://{domain}:8888/scrape")
    print(f"✅ CORS Test: https://{domain}:8888/cors-test")
    
    print(f"\n📝 Next steps:")
    print("1. Update your frontend to use: https://mininglifeserver.com:8888/scrape")
    print("2. Update backend CORS in main.py to include the new domain")
    print("3. Restart your PM2 backend: pm2 restart job-scraping-backend")
    print("4. Test the mixed content issue is resolved")
